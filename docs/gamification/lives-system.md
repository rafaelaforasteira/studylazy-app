...
- **Tela de revisão de erros não consome vidas** (UX deliberada — documentada).

### Offline

Estado persistido em AsyncStorage (`studylazy-lives`). Regeneração é calculada no cliente com timestamps locais.

## Free / Pro

- Free: 5 vidas com regeneração.
- Pro (futuro / `devProEnabled`): `isUnlimited = true` — não bloqueia estudo.
- Tela `/pro` lista “Vidas ilimitadas para estudar sem esperar.” sem compra.
...
4. Errar questões → ver badge “Revisão de erro” na sessão seguinte da mesma matéria
5. Acertar a questão em retry → sai da prioridade máxima
...
- Regeneração só por tempo (sem anúncios)
- Retry é local (não sincroniza fila via Supabase nesta versão)
- Revisão dedicada não gasta vidas
